...
export const onDel = (currentItem: Item[], delItem: string) => {
  const result = currentItem.filter((item: Item) => item._id !== delItem);
  return result;
};

export const addComment = (currentItem: Item[], newItem: Replies, target: string) => {
  const itemIndex = currentItem.findIndex((item: Item) => item.link.toLowerCase() === target);
...
export const upVote = (currentItem: Item[], newItem: Item) => {
  ...
  if (exist) {
    const voted = currentItem.map((item: Item) =>
      item.title === newItem.title ? { ...exist, vote: exist.vote + 1, voted: true } : item
    );
    return voted;
...
export const downVote = (currentItem: Item[], newItem: Item) => {
  ...
  if (exist) {
    const voted = currentItem.map((item: Item) =>
      item.title === newItem.title ? { ...exist, vote: exist.vote - 1, voted: false } : item
    );
    return voted;
  } else return currentItem;
};
